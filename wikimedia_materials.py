...
UYDURULMUS_KATEGORILER = (
    "colorized images",
    "colourized images",
    "ai upscaler software unknown",
    "images upscaled with ai",
    "ai-upscaled images",
    "images modified by ai",
    "ai-generated images",
    "images generated by artificial intelligence",
)
"""Goruntusu SONRADAN URETILMIS dosyalarin Commons kategorileri.
...
buyutulmustu. Commons'in kendi uyarisi:
...
⚠️ Isaret ZATEN CEKILIYORDU: `extmetadata.Categories` (`iiprop`e dahil), yani
yeni bir ag istegi gerekmiyor. Uc dosyada dogrulandi:

    Cemal Paşa ve ailesi.jpg  -> "AI upscaler software unknown|Colorized images"
    Jamal Pasha inspecting…   -> "Retouched pictures|Colorized images"
    temiz kontroller          -> hicbiri yok

⚠️ "Retouched pictures" BILEREK YOK. O kategori kirpma ve toz temizligini de
kapsiyor, yani dogru bir arsiv taramasini da elerdi. Listede yalnizca
goruntuyu UYDURAN islemler var.

⚠️ Eslesme kategori adinin TAMAMINA yapiliyor, ALT DIZEYE degil: "ai " dizesi
"Sinai (…)" icinde de geciyor ve alt dize aramasi Sina Yarimadasi
fotograflarini elerdi. Bunu olcerken kendi tarama betigim tam bu yanlis alarmi
verdi.
"""


def uydurulmus_goruntu(kategoriler: str) -> str:
    """Goruntuyu SONRADAN ureten bir islem uygulanmis mi — eslesen kategori adi.

    Commons `extmetadata.Categories` alanini "A|B|C" bicimde veriyor.
    """
    parcalar = {
        parca.strip().lower()
        for parca in str(kategoriler or "").split("|")
        if parca.strip()
    }
    for isaret in UYDURULMUS_KATEGORILER:
        if isaret in parcalar:
            return isaret
    return ""
...
def _puanli_adaylar(
    ...
    for order, page in enumerate(pages):
        ...
        if not kullanilabilir_lisans(license_name):
            continue
        # ⚠️ GORUNTUSU SONRADAN URETILMIS dosyalar — gerekce
        # `UYDURULMUS_KATEGORILER`de. Suzgec LISANSIN YANINDA duruyor cunku
        # ikisi de ayni soruyu soruyor: bu dosya videoya girebilir mi.
        #
        ...
        # `_kategori_adaylari`, tekil sayfa yolu). Yani tek suzgec her yolu
        # kapatiyor. Yalnizca menuye konsaydi kategori havuzundan gelen
        # ikincil gorseller acikta kalirdi.
        #
        # ⚠️ Eleme LOGLANIYOR: capa havuzunun sessizce tukenmesi bu depoda
        # olculmus bir kusur (0 uygun capa = uretim durur ama log "red" der).
        if uydurulmus := uydurulmus_goruntu(_metadata_value(image, "Categories")):
            print(f"ℹ️ elendi ({uydurulmus}): {title}", flush=True)
            continue
        url = image.get("url")
        if not url:
            continue
